...
static void runOnMainThread(int handle) {
...
static void loadAssetInThread(int handle) {
  ...
  phonePostToMainWorkQueue(notifyWorkItem);
}
...
int phoneMain(int argc, const char *argv[]) {
...

  {
    ...
    thread = phoneCreateThread("loadAssetThread",
      loadAssetInThread);
    phoneSetHandleTag(thread, ctx);

    notifyWorkItem = phoneCreateWorkItem(0, runOnMainThread);
    phoneSetHandleTag(notifyWorkItem, ctx);

    phoneStartThread(thread);
  }

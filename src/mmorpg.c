...
void sprite2dRender(sprite2d *sprt, imageFrame2d *frame) {
  float onCanvasLeft = sprt->x / (game->width) - 1;
  float onCanvasTop = sprt->y / (game->height) - 1;
  float onCanvasRight = (sprt->x + frame->width) / (game->width) - 1;
  float onCanvasBottom = (sprt->y + frame->height) / (game->height) - 1;
  float texLeft = frame->left / frame->tex->width;
  float texTop = frame->top / frame->tex->height;
  float texRight = (frame->left + frame->width) / frame->tex->width;
  float texBottom = (frame->top + frame->height) / frame->tex->height;
  vertex vertices[4] = {
...
static duk_ret_t ImageFrame2D_rotate(duk_context *js) {
  imageFrame2d *frame = getRawPointerFromThis(js);
  int degree = duk_to_int(js, 0);
  imageFrame2dRotate(frame, degree);
  return 0;
}

static duk_ret_t ImageFrame2D_setWidth(duk_context *js) {
  imageFrame2d *frame = getRawPointerFromThis(js);
  int width = duk_to_int(js, 0);
  imageFrame2dSetWidth(frame, width);
  return 0;
}

static duk_ret_t ImageFrame2D_setHeight(duk_context *js) {
  imageFrame2d *frame = getRawPointerFromThis(js);
  int height = duk_to_int(js, 0);
  imageFrame2dSetHeight(frame, height);
  return 0;
}

static duk_ret_t ImageFrame2D_setCenter(duk_context *js) {
  imageFrame2d *frame = getRawPointerFromThis(js);
  int x = duk_to_int(js, 0);
  int y = duk_to_int(js, 1);
  imageFrame2dSetCenter(frame, x, y);
  return 0;
}
...
static void pushImageFrame2dFromRawPointer(duk_context *js,
    imageFrame2d *frame) {
  duk_push_object(js);
  duk_push_c_function(js, ImageFrame2D_rotate, 1);
  duk_put_prop_string(js, -2, "rotate");
  duk_push_c_function(js, ImageFrame2D_setWidth, 1);
  duk_put_prop_string(js, -2, "setWidth");
  duk_push_c_function(js, ImageFrame2D_setHeight, 1);
  duk_put_prop_string(js, -2, "setHeight");
  duk_push_c_function(js, ImageFrame2D_setCenter, 2);
  duk_put_prop_string(js, -2, "setCenter");
...
static void layout(void) {
  ...
  game->width = openGLViewWidth;
  game->height = openGLViewHeight;
}
